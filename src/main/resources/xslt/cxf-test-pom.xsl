...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/pom:project[./pom:artifactId/text() = 'quarkus-cxf-integration-test-server']/pom:profiles/pom:profile[./pom:id/text() = 'native-image']/pom:build/pom:plugins/pom:plugin[./pom:artifactId/text() = 'maven-failsafe-plugin']/pom:configuration">
        <xsl:copy>
            <xsl:apply-templates select="@* | node()" />
            <systemPropertyVariables>
                <quarkus.platform.group-id>${project.groupId}</quarkus.platform.group-id>
